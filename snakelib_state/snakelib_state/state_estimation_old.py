...
class StateEstimator(Node):
    ...
    def update_plot(self):
        self.ax.clear()
        T_list = self.compute_fk(self.dh, self.current_joint_angles)
        origins = np.array([T[:3, 3] for T in T_list]) # Step 1
        ...
        self.ax.scatter(coms[:, 0], coms[:, 1], coms[:, 2], c='m', label='Link COMs')
        self.ax.scatter(*overall_com, c='cyan', s=100, label='Overall COM')
        self.ax.text(*overall_com, 'COM', color='cyan', fontsize=10)
        ...
        self.ax.quiver(*T_vc[:3, 3], *T_vc[:3, 0], color='r')  # X axis
        self.ax.quiver(*T_vc[:3, 3], *T_vc[:3, 1], color='g')  # Y axis
        self.ax.quiver(*T_vc[:3, 3], *T_vc[:3, 2], color='b')  # Z axis
        self.ax.text(*T_vc[:3, 3], 'Virtual Chassis', fontsize=10)
        ...
        self.ax.set_title('Snake Shape With Respect To Tail')
